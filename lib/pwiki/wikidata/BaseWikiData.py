...
class BasicWikiData:
    ...
    def backupDatabase(self):
        ...
        answer = wx.MessageBox(_("WikidPad can make a backup of "
                ...
                "CANCEL to skip."
                "Backup will be created at: {0}".format(dbBackupPath)), 
                _('Backup database?'),
                wx.OK | wx.CANCEL | wx.ICON_QUESTION, self)
        if answer == wx.OK:
            for orig, new in self.getDbFilenames():
                dbPath = join(self.dataDir, orig)
                dbBackupPath = join(self.dataDir, strftime("%Y-%m-%d_%h-%M"), 
                        orig)

            shutil.copyfile(dbPath, dbBackupPath)
    ...
    def _getCachedWikiPageLinkTermDict(self):
        ...
        class CachedWikiPageLinkTermDictNoCase(object):
            ...
            def get(self, key, default=None):
                ...
                if self.cache.has_key(key):
                    return self.cache.get(key, default)
                elif self.cacheLower.has_key(key):
                    return self.cacheLower.get(key, default)
